...
/* Wrapper functions of system calls */
/* I/O wrapper functions */
int Open(const char *pathname, int flags) {
    /* 
     * Ignore mode here since we do note need to manage
     * user authentication
     */
...
/* Helper functions for waiting foreground job to terminate */
...
void eval_exe(struct cmdline_tokens tok, int bg, char* cmdline) {
    ...
    /* Block signals to avoid SIGINT before addjob */
 ...
 * Restarts job by sending it a SIGCONT and then runs 
...
void eval_bg(struct cmdline_tokens tok) {
    ...
    /* Output logs and send SIGCONT to the job */
...
void eval_fg(struct cmdline_tokens tok) {
    ...
    /* Evaluate next cmdline until foreground job terminates */
    Sigprocmask(SIG_BLOCK, &mask_all, &prev_mask);
    wait_fg(prev_mask);
    Sigprocmask(SIG_SETMASK, &prev_mask, NULL);
}
 ...
 * Makes the trailing command ignore any SIGHUP signals
 */
void eval_nohup(char* cmdline) {
    sigset_t mask, prev_mask;
    int offset = strlen("nohup");
    Sigemptyset(&mask);
...
eval(char *cmdline) 
{
    ...
    /* Redirect I/O */
    int stdin_fd, stdout_fd, in_fd, out_fd;
    if (tok.infile) {
    ...
    if (tok.outfile) {
        out_fd = Open(tok.outfile, O_WRONLY | O_TRUNC | O_CREAT);
...
sigchld_handler(int sig) 
{
    ...
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0) {
        job = getjobpid(job_list, pid);
        if (WIFEXITED(status)) {
            /* Normallt exited, delete */
            deletejob(job_list, pid);
        }
        else if (WIFSTOPPED(status)) {
            /* Stopped, output log and change state */
            int jid = pid2jid(pid);
            ...
            /* Terminated, output log and delete*/
            int jid = pid2jid(pid);
            ...
            /* Continued from outside the shell */
            job->state = BG;
        }
    }
    errno = old_errno;
